...
namespace GneissBooks.ViewModels
{
    ...
    {
        public string Title => $"{TransactionId ?? ""}, {Date: yyyy-MM-dd}: {Description}, {Math.Abs(Lines.FirstOrDefault()?.Amount ?? 0).ToString("F2")} {Lines.FirstOrDefault()?.CurrencyCode ?? "NOK"}";
